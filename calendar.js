  ...
  try {
    ...
    setSyncState('error',
      `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
       Impossible de joindre Google Sheets — données locales utilisées`
    );
    render();
  }
}
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      if (!info) {
        ...
        const color = info.booking.isOwner ? 'var(--green)' : 'var(--blue)';
        ...
        const c1 = info.prevBooking.isOwner ? 'var(--green)' : 'var(--blue)';
        const c2 = info.booking.isOwner     ? 'var(--green)' : 'var(--blue)';
...
function renderList(year) {
  ...
  let html = '';
  filtered.forEach(b => {
    const color  = b.isOwner ? 'var(--green)' : 'var(--blue)';
...
(async function init() {
  ...
  try {
    ...
    const notConfigured = err.message && err.message.includes('non configurée');
    setSyncState('error',
      notConfigured
        ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
           Synchro non configurée — <a href="setup.html" style="color:inherit;font-weight:600;">Suivre le guide de configuration →</a>`
        : `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
           Données locales — <a href="#" onclick="syncFromSheet();return false;" style="color:inherit;font-weight:600;">Réessayer</a>`
    );
  }
})();
